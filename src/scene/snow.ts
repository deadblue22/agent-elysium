...
import {
  BufferGeometry, Float32BufferAttribute, NormalBlending, Points, Quaternion, ShaderMaterial, Vector3,
...
export function createSnow(camera: PerspectiveCamera, avoid: Rect | null, count = 230) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (kind === 0) { r0 = 0.7 + R() * 1.5; a = 0.35 + R() * 0.45; vy = 10 + R() * 16; depth = 14 + R() * 16; }
    else if (kind === 1) { r0 = 2 + R() * 1.8; a = 0.35 + R() * 0.3; vy = 18 + R() * 14; depth = 10 + R() * 12; }
    else { r0 = 7 + R() * 9; a = 0.07 + R() * 0.08; vy = 26 + R() * 12; depth = 6 + R() * 5; }
  ...
  const material = new ShaderMaterial({
    ...
    vertexShader: /* glsl */ `
      ...
      void main() {
        ...
        float d = aSeed.z;
        float k = d / 3200.0; // world units per frame px at this depth (the frame is 32 units from the eye)
        vec3 p = uEye + uFwd * d + uRight * ((sx - 800.0) * k) + uUp * ((100.0 - sy) * k);
