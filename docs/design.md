...
> M0 风格板证明 CSS 3D 无法同时让文字清晰并贴合纸面透视，且运行时 SVG 滤镜性能不足。评估与推荐见 `docs/tech-eval.md`：迁移到 Three.js，纸片纹理离线烘焙，文字页以画布纹理贴在页面网格上，投影与光照交给渲染器。下表为 M0 之前的原始选型，待确认后按评估结果更新。

| 层 | 选择 | 理由 |
|---|---|---|
| 构建 | Vite + TypeScript | 零配置，静态输出，可直接部署到 GitHub Pages |
| 视图 | 原生 DOM + 内联 SVG | 剪纸形状天然适合 SVG，无需框架 |
| 3D 感 | CSS `perspective` + `transform` | 背景板竖起与视差只需要 2.5D，不引入 Three.js |
| 动画 | Web Animations API，必要时 GSAP | 序列动画用 GSAP 的时间线更省事 |
| 字体 | 自托管子集 | 避免运行时依赖 Google Fonts |
