...
选项
: 1. 「走吧，去二楼。」/ "Let's go. Second floor." → study.end
...
金转身向右走出房间，哈里跟在后面，两人在页边折倒。窗开着，蜡烛随后熄灭，房间只剩窗外的雪光。日志最后一行居中显示「第一章 完」/ "End of Chapter One"。
...
- 选择带检定的选项后，左页文字暂停，两枚纸骰子从右侧掷到书旁的桌面上，翻滚约 0.8 秒后停下。
...
│  木桌                                                           │
│    ┌───────────── 立体背景板（书房后墙）──────────────┐              │
│    │ 书架    窗（窗外飘雪）   相框   镜子 + 钟    日历   │              │
│    │ 扶手椅    书桌 + 死者 + 蜡烛    壁炉     暖气片   │   线索卡堆    │
│    ├── 左页撕口 ──┐        地毯          衣帽架     │              │
│    │ 左页：文字日志 │    哈里        金               │   ♥ ♥ ♥ ♡    │
│    │ ……           │                                │              │
│    │ 1. ……▮       ├────────── 右页撕口 ──────────────┤    ⚂ ⚄       │
...
M1 之后第二次打磨（PR #1）起的布局。此前的版本俯角为 68°–74°，书占满画面，人物站在右页白纸上，见 `docs/m1-*.png` 的 Git 历史。

- 相机：俯角约 58°，接近坐在桌前看书的角度，与参考 demo 相当。镜头为长焦（焦距 2600 帧像素，1600 × 900 画面），整本书和背景板完整入画，四周露出桌面。书略偏左，右侧桌面留给骰子、士气与线索卡。
- 书页：单页 670 × 720，略高于宽，接近真实书本比例。
- 左页：顶层纸撕到约四分之一深度。撕口后方的地板上从远到近依次立着书架与壁炉、书桌、扶手椅；撕口以下是文字日志。
- 右页：顶层纸撕到接近页尾，只剩一条纸边和页码，房间地板一直铺到那里。哈里与金站在房间中央的地毯上，是房间的前景；回忆时楼梯从壁炉与衣帽架之间升起。右页因此是舞台本身，不再承载界面元素。
- 桌面：书右侧从远到近依次放线索卡堆、四颗纸心（士气）、两枚骰子。悬停可查看士气数值与已找到的线索。
- 文字：画面中汉字约 16 × 18 像素（高 × 宽，1600 宽画面），一屏约 10 行。左页相对视线倾斜约 55°，字形会被压扁；文字画布纵向拉伸 1.1 倍补偿，屏幕上的字形高宽比约 0.89。
- 背景板各层与纸偶以 9°–18° 后倾竖立（越远越倾），鼠标移动时相机绕书中心小幅转动（水平 ±3°、垂直 ±1.4°），形成视差。
- 雪只在窗外下：雪片位于后墙与窗外远景之间，只能透过窗洞看到。

布局示意图 `docs/mockup-layout.svg` 是方案阶段的草图，只用于确认配色关系。
...
| 新线索 | 日志中插入一行系统提示「新线索　指针被拨过（1/3）」；右页前部落下一张贴着胶带的大纸卡。点击后纸卡缩小，飞到书右侧桌面上的线索卡堆，叠在已找到的线索上 |
| 内心声音 | 技能名以属性色出现，文字前有 0.3 秒停顿 |
| 主动检定 | 骰子从右侧掷到桌面上翻滚，结算行出现 |
| 士气变化 | 桌面上的纸心依次翻面：失去一点时，红心跃起、翻转，落下时变成灰白 |
| 还原成功 | 画面转为冷色夜景，窗外的雪停下，时间标记显示真实时间（22:30 起，逐段推进到 23:30）。楼梯从地板升起，狗趴在楼梯脚下；马雷克上楼，从楼梯后面走进房间，依次走到书桌、座钟、窗前，再原路下楼；钟的指针转到 23:40，窗户打开，最后开始下雪 |
| 还原失败 | 背景板短暂变暗后恢复，雪片不出现 |
| 章节结束 | 金转身、哈里跟上，两人向右走出房间并在页边折倒；蜡烛熄灭，画面略微变暗，日志末尾出现「第一章 完」 |
...
M1 之后的第一次打磨（提交 37792f4）：放大日志、逐段推进、减淡纸纹、加强光影、突出新线索、加入悬停提示。

M1 之后的第二次打磨（视角、文字、右页，PR #1）：已完成。

- 视角：从近乎垂直的 74° 俯视改为约 58°，整本书入画，四周露出桌面（6.1 节）。
- 书页：加深到 670 × 720。左页撕口下移，后方的书架、书桌、扶手椅之间留出地板；右页撕到页尾附近，变成房间地板。
- 文字：画面中约 16 像素高（此前 20–22 像素），纵向拉伸 1.1 倍补偿斜看造成的压扁。
- 右页：哈里与金站进房间；骰子、士气、线索卡移到书旁的桌面上。
- 回忆：马雷克在房间里走动，不再瞬移；楼梯与狗放大；时间标记按段显示真实时间。
- 雪只在窗外下。
- 结尾：翻页改为两人离场、熄烛，日志末尾出现「第一章 完」。
- 悬停提亮加强，「▼ 继续」放大。

7.2 节的模块划分是方案阶段的规划，实际模块见 README。
